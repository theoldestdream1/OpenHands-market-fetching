...
from candle_fetcher import fetch_candles, fetch_initial_history
...
class CandleScheduler:
    ...
    async def _fetch_pair_at_offset(self, pair, delay):
        """Fetch all timeframes for a pair after a delay."""
        ...
        for timeframe in TIMEFRAMES:
            ...
            if candles and len(candles) > 0:
                ...
                print(f"Loaded 1 new candle for {pair}/{timeframe}")

            api_key_manager.record_usage(key)

    async def minute_tick(self):
        """Runs once per minute and schedules time-sliced fetches for all pairs."""
        if not self.is_initialized:
            return

        now = datetime.now(timezone.utc)
        print(f"[{now.isoformat()}] Scheduling fetches for all timeframes")

        for pair in PAIRS:
            offset = self.pair_offsets.get(pair, 0)
            asyncio.create_task(self._fetch_pair_at_offset(pair, offset))
    ...
    def start(self):
        ...
        # Run minute_tick every minute at second 0
